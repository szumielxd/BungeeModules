    	...
    	if(args.length == 0) {
            ...
            try {
                ...
                if (sender instanceof ProxiedPlayer) {
                    sender.sendMessage(new ComponentBuilder(ProxyServer.getInstance().getTranslation("error_occurred_player")).event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, 
                    		new Text(new ComponentBuilder(error.getMessage()).color(ChatColor.RED).create())
                    )).create());
                } else {
